...
export function getTokenLOSheight(token) {
    ...
    if (WallHeight._autoLosHeight) {
        losDiff = token.document.flags[MODULE_SCOPE]?.tokenHeight || WallHeight._losHeightMulti * canvas.scene.dimensions.distance * Math.max(token.document.width, token.document.height) * ((Math.abs(token.document.texture.scaleX) + Math.abs(token.document.texture.scaleY)) / 2);
    } else {
    ...
    for (const wall of walls) {
        ...
        if ((oldTop !== null && oldTop !== undefined) || (oldBottom !== null && oldBottom !== undefined)) {
            ...
            if (wall.flags["token-attacher"]) {
                ...
                if ((oldTop !== null && oldTop !== undefined) || (oldBottom !== null && oldBottom !== undefined)) {
                    foundry.utils.setProperty(update, `flags.token-attacher.offset.elevation.flags.wall-height`, {
                        top: oldOffsetTop,
                        bottom: oldOffsetBottom,
                    });
                    foundry.utils.setProperty(update, `flags.token-attacher.offset.elevation.flags.-=wallHeight`, null);
                }
            }
            updates.push(update);
        }
