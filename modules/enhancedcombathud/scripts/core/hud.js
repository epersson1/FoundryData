...
export class CoreHUD extends Application {
    constructor() {
        ...
        this._batchUpdateItemButtons = foundry.utils.debounce(this._batchUpdateItemButtons, 200);
    ...
    setPosition(preventTransition = true) {
        ...
        const scale = !game.settings.get("enhancedcombathud", "autoScale") ? game.settings.get("enhancedcombathud", "scale") : window.innerHeight / 2000; //(1 / (echHUDWidth / windowWidth)) * game.settings.get("enhancedcombathud", "scale");
    ...
    interceptNextDialog(element) {
        ...
        const hookId = Hooks.once("renderDialog", (dialog, html) => {
            ...
            offset.left += element.getBoundingClientRect().width + 10 - window.scrollX;
